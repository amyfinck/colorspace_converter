...
void allocate_rgb_pixels_memory(RGB_image_t *rgb_img)
{
    ...
    exit_on_error(rgb_img->pixels == NULL, "Error: Malloc for RGB pixels failed");
}
...
void allocate_ycc_pixels_memory(YCC_image_t *ycc_img)
{
    ...
    exit_on_error(ycc_img->pixels == NULL, "Error: Malloc for YCC pixels failed");
}

void copy_rgb_to_yyc(RGB_image_t *rgb_in_img, YCC_image_t *ycc_out_img)
{
    ycc_out_img = malloc(sizeof(YCC_image_t));
    exit_on_error(ycc_out_img == NULL, "Error: Malloc for YCC image failed");
...
{
    rgb_out_img = malloc(sizeof(RGB_image_t));
    exit_on_error(rgb_out_img == NULL, "Error: Malloc for RGB image failed");
